...
public class AILevelThree : Player {
    ...
    public override void PlayerChess()
    {
        ...
        {
            CreateTree(item, (int[,]) ChessBoard.Instance.grid.Clone(), 3, false);
    ...
    List<MiniMaxNode> GetList(int[,] grid,int chess,bool mySelf)
    {
        ...
        for (int i = 0; i < 15; i++)
        {
            for (int j = 0; j < 15; j++)
            {
                ...
                if (nodeList.Count < 4)//4是推演步数，可以自己定义
                {
                    nodeList.Add(node);
                }
                else
...
{
    public int chess; //棋颜色
    public int[] pos; //棋位置
    public List<MiniMaxNode> child; //分支 
    public float value;   //分数
}